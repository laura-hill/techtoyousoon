...
Imagine that you are working in Gaming company as a DevOps Engineer. Recently, you have deployed a new version of the firm's famous Game and its content is being kept under `/home/ec2-user/data` in minute basis. In addition, your manager asked you to back up `/etc`, `/boot`, and `/usr`. So, `/home/ec2-user/data`, `/etc`, `/boot`, and `/usr` folders need to be backed up and placed in a specific folder `/mnt/backup` every 5 minutes. At the same time, since you should separate each file from others, you have to assign different names to backup files based on the hostname of instance and the date-hour-minute of the backup. Each backup file has to be compressed with `.tgz` extension. You can not manage this dynamic process by yourself. You decided to create a bash script and define cronjob to execute this every 5 minutes. Files which are  `/home/ec2-user/data`, `/etc`, `/boot`, `/usr` have to be backed up. Please write a bash script named `backup.sh` for this task.
