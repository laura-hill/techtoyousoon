...
- Application is coded by Clarusway Fullstack development team and given to you as DevOps team. The app allows users to write their own blog page, whose user registration data should be kept in separate MySQL database in AWS RDS service and pictures or videos should be kept in S3 bucket. The object list of S3 Bucket containing movies and videos is recorded on DynamoDB table. 
...
- You are requested to push your program to the project repository on Github. You are going to pull it into the webservers in the production environment on AWS Cloud. 
...
In the architecture, you can configure your infrastructure using the following:
  ...
  - Specifications of VPC:
    ...
    - VPC has two AZs and every AZ has 1 public and 1 private subnet.

    - VPC has Internet Gateway

    - One of the public subnets has a NAT Instance.

    - You might create new instance as Bastion host on Public subnet or you can use a NAT instance as Bastion host.

    - There should be a managed private and public route table.

    - Route tables should be arranged according to routing policies and subnet associations based on public and private subnets.

  - You should create an Application Load Balancer with Auto Scaling Group of Ubuntu 18.04 EC2 Instances within a created VPC.
  ...
  - The Auto Scaling Group should use a Launch Template in order to launch instances needed and should be configured to:
    ...
    - use all Availability Zones on a created VPC.
  ...
  - The Launch Template should be configured to:
    ...
    - Install the requirements using requirements.txt in the 'aws_capstone' folder
  ...
  - As S3 Bucket
    ...
    - First S3 Bucket
      ...
      - Since development team doesn't prefer to expose traffic between S3 and EC2s on internet, Endpoint should be set on a created VPC. 
